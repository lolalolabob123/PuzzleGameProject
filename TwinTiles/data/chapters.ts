    ...
    if (chapterId === 2) {
      ...
      let empties = finalGrid
        ...
        for (let l = 0; l < numLinks; l++) {
          ...
          // Pick random empty candidate
          const firstIndexEmpties = Math.floor(linkRng() * empties.length);
          const first = empties[firstIndexEmpties];

          // Find a matching index with same solution color
          const matchIdx = empties.findIndex((idx) => idx !== first && solution[idx] === solution[first]);

          if (matchIdx !== -1) {
            const second = empties[matchIdx];
            // Remove unused indices
            empties = empties.filter((idx) => idx !== first && idx !== second);
            links.push({
              indices: [first, second],
              color: LINK_COLORS[l % LINK_COLORS.length],
            });
          }
        }
    }
