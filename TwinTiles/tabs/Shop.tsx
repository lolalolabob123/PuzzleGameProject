...
export default function Shop() {
  ...
  const handleBuy = async (item: ShopItem) => {
    ...
    if (coins < item.price) {
      universalNotify(
        "Not enough coins",
        `You need ${item.price - coins} more.`
      );
      return;
    }
  ...
  const { cardWidth } = useMemo(() => {
    ...
    const cols =
      available < 540 ? 2 : available < 820 ? 3 : 4;

    const totalGap = gap * (cols - 1);
    const card = Math.floor((available - totalGap) / cols);
    return { cardWidth: card };
  }, [windowWidth]);
...
const ThemePreview = ({ theme }: { theme: GameTheme }) => (
  ...
  >
    <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: theme.shape1Color }} />
    <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: theme.shape2Color }} />
    <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: theme.shape2Color }} />
    <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: theme.shape1Color }} />
  </View>
);
...
const ThemePreviewSheet = ({
  ...
  // Enforces valid checkerboard pattern (max 2 adjacent of same color)
          ...
          >
            <View
              style={{
                ...
                borderRadius: 16,
                backgroundColor:
                  val === 1 ? theme.shape1Color : theme.shape2Color,
              }}
            />
          </View>
        ))}
...
const ItemCard = ({
  ...
  const disabled = !isTheme && (isOwned || !canAfford);

  return (
    <TouchableOpacity
      style={[styles.card, { width: cardWidth }, disabled && styles.cardDisabled]}
      onPress={() => {
        if (isTheme) onPreview();
        else onBuy();
      }}
      disabled={!isTheme && isOwned}
      activeOpacity={0.85}
    >
      <View style={styles.cardIconWrap}>
        {isTheme && themeById(item.id) ? (
